...
This demo application demonstrates a simple polyphonic synthesizer using *nRF5340 DK* or *nRF5340 Audio DK*. Synthesized audio is transmitted with le audio to receiving *nRF5340 Audio DK*s, which functions as the headphone.
...
Programming left headphone board, snr is inferred if only one device is connected. If not, you will be promted to select from a list
> python program.py --device left

#### Building and program yourself
The application is developed using `nrf-sdk v2.0.2`. `common_net.hex` is borrowed from the `nrf5340_audio` application provided with this version of nrf-sdk, located in `bin/ble5-ctr-rpmsg_3251.hex`. The headphones has to be programmed with this audio application version to function correctly. The provided headphone binaries are build from the stock `nrf5340_audio` application with added configs `CONFIG_AUDIO_HEADSET_CHANNEL_COMPILE_TIME=y` and `CONFIG_AUDIO_HEADSET_CHANNEL=x`, where x=0 => left headphone, x=1 => right headphone.
...
### Testing
Turn on both the synthesizer board as well as 1 or 2 headphone boards. If `LED1`(blue) lights up on the headphone board, there is a connection. By pressing the `PLAY/PAUSE` button on the headphone board, audio output is enabled (the blue light should flash). Connect speaker to the headphone aux connection on the headphone. Pressing `Button 1` to `Button 4` on the synthesizer board should now result in audio from headphone boards.
